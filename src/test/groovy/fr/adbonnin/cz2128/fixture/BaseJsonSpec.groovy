...
abstract class BaseJsonSpec extends Specification {
    ...
    static final Json DEFAULT_CZ2128 = new Json(DEFAULT_MAPPER)

    Path tempDir

    void setup() {
        tempDir = Files.createTempDirectory(getClass().simpleName).toAbsolutePath()
    }
    ...
    Json.ProviderFactory newMemoryProviderFactory(String content = "") {
        return DEFAULT_CZ2128.memoryProvider(content)
    }

    Json.ProviderFactory newFileProviderFactory(String content) {
    ...
    ContentProvider newMemoryProvider() {
        return new MemoryProvider(DEFAULT_MAPPER.getFactory())
    }

    ContentProvider newFileProvider() {
        return new FileProvider(newTempFile(), DEFAULT_MAPPER.getFactory())
    }

    Path newTempFile() {
        def subDir = Files.createTempDirectory(tempDir, 'sub-dir')
        return subDir.resolve('test.json')
    }
